...
const Header = () => {
  ...
  return (
    <header
      className={`sticky top-0 left-0 right-0 backdrop-blur-md z-50 shadow-sm ${
      ...
      <div className='relative flex items-center justify-between px-4 py-2 max-w-[1280px] mx-auto'>
        ...
        <div className='hidden lg:flex flex-1 justify-center gap-1'>
          {menuData.map((item, i) => (
            <p
              key={i}
              className={`text-md font-semibold px-3.5 py-2 rounded-full transition-colors cursor-pointer hover:bg-gray-100 ${
